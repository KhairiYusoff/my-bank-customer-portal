...
export type CompleteProfileFormData = {
  ...
  educationLevel: 'none' | 'primary' | 'secondary' | 'diploma' | 'degree' | 'postgraduate';
  ...
  job: string;
  employerName: string;
  employmentType: 'salaried' | 'self-employed' | 'unemployed' | 'retired' | 'student';
  salary: '<1000' | '1000-2999' | '3000-4999' | '5000-6999' | '7000-9999' | '10000+';
  accountType: 'savings' | 'current' | 'fixed deposit';
  purposeOfAccount: 'savings' | 'salary credit' | 'investment' | 'business' | 'education' | 'travel' | 'other';

  // Next of Kin
  nextOfKin: {
    name: string;
    phone: string;
    relationship: 'parent' | 'spouse' | 'child' | 'sibling' | 'relative' | 'friend' | 'other';
  };
};
...
export const completeProfileSchema = yup.object<CompleteProfileFormData>().shape({
  // Personal Information
  password: yup
    .string()
    .required('Password is required')
